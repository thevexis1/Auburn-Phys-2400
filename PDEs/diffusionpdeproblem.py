...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  8 10:48:44 2025
...
L = np.kron(I, T) + np.kron(np.diag(e[:-1], k=1) + np.diag(e[:-1], k=-1), I)
L = L / (hx**2)  # scale by grid spacing


subsection_size = 50  
L_original = L * (hx**2)
L_sub = L_original[:subsection_size, :subsection_size]
plt.figure(figsize=(10, 8))
plt.imshow(L_sub.T, cmap='coolwarm')
plt.colorbar(label='Matrix Value')
plt.title(f'2D Finite Difference Laplacian Matrix (Top {subsection_size}x{subsection_size})')
plt.xlabel('Column')
plt.ylabel('Row')
...
u0 = np.exp(-100 * ((xx - 0.5)**2 + (yy - 0.5)**2))  # Gaussian bump at center
u = u0.flatten()
...
def update(frame):
    ...
    ax.set_title(f"Time: {frame * 20 * dt:.4f} s  2D-Diffusion")
...
for step in range(steps):
    u_hat = u_hat * np.exp(-kappa * K_magnitude * dt)
    if step % 10 == 0:
...
def update(frame):
    ...
    ax.set_ylabel('y')
    ax.set_zlabel('Temperature')
    ax.set_title(f"Time: {frame * 20 * dt:.3f} s  2D-Diffusion)")
